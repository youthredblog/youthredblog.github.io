...
- 需要主要的是，至本片文章编写之日，`JSch` 版本 `0.1.53` 及以下都已发现 [漏洞](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2016-5725) 。最新 `0.1.55` 无漏洞。
...
public class SshForward {
    ...
    public String[] exec(String... cmd) {
        if (ArrayUtils.isNotEmpty(cmd)) {
            String[] resultPrints = new String[cmd.length];
            for (int i = 0; i < cmd.length; i++) {
                resultPrints[i] = JschUtil.exec(getTarget(), cmd[i], StandardCharsets.UTF_8);
            }
            return resultPrints;
        }
        return null;
...
try {
    SshForward forward = new SshForward(hosts);
